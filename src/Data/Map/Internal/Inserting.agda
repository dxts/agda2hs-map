...
module Inserting {k a : Set} ⦃ iOrdk : Ord k ⦄ where
  ...
  insert kx x t@(Bin sz ky y l r) = match (compare kx ky)
    where
      match : (o : Ordering) → Map k a
      match LT = let l' = insert kx x l in if (l' == l) then t else (balanceL ky y l' r)
      match GT = let r' = insert kx x r in if (r' == r) then t else (balanceR ky y l r')
      match EQ = if (x == y) then t else (Bin sz kx x l r)
  {-# COMPILE AGDA2HS insert #-}

  insertR :  ⦃ Eq a ⦄ → ⦃ Eq (Map k a) ⦄ → k → a → (m : Map k a) → Map k a
  insertR kx x Tip                 = singleton kx x
  insertR kx x t@(Bin sz ky y l r) = match (compare kx ky)
    where
      match : (o : Ordering) → Map k a
      match LT = let l' = insert kx x l in if (l' == l) then t else (balanceL ky y l' r)
      match GT = let r' = insert kx x r in if (r' == r) then t else (balanceR ky y l r')
